...
def cnn_layer(embedded_x, max_seq_len, num_filters, filter_size, is_training, dropout_rate,
              reuse=False):
...
def cnn_layers(embedded_x, max_seq_len, num_filters, filter_sizes, is_training, dropout_rate,
               reuse=False):
    pooled_flats = []
    with tf.variable_scope('cnn_network', reuse=reuse):
        for i, (n, size) in enumerate(zip(num_filters, filter_sizes)):
            with tf.variable_scope('cnn_layer_{}'.format(i), reuse=reuse):
                pooled_flat = cnn_layer(embedded_x, max_seq_len, num_filters=n, filter_size=size,
